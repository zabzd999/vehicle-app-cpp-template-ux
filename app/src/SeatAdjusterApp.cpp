...
const auto TOPIC_REQUEST_COPILOT          = "seatadjuster/setPosition/request_coPilot";
const auto TOPIC_RESPONSE_COPILOT         = "seatadjuster/setPosition/response_coPilot";
const auto TOPIC_CURRENT_POSITION_COPILOT = "seatadjuster/currentPosition_coPilot";
...
void SeatAdjusterApp::onStart() {
    ...
    subscribeDataPoints(
        velocitas::QueryBuilder::select(Vehicle.Cabin.Copilot.Seat.Row1.DriverSide.Position)
            .build())
        ->onItem([this](auto&& item) { onSeatPositionChanged(std::forward<decltype(item)>(item)); })
        ->onError(
            [this](auto&& status) { onErrorDatapoint(std::forward<decltype(status)>(status)); });
    ...
    subscribeToTopic(TOPIC_REQUEST_COPILOT)
        ->onItem([this](auto&& item) {
            onSetPositionRequestReceived(std::forward<decltype(item)>(item));
        })
        ->onError([this](auto&& status) { onErrorTopic(std::forward<decltype(status)>(status)); });
...
void SeatAdjusterApp::onCoPilotSeatPositionChanged(const velocitas::DataPointReply& dataPoints) {
    ...
    try {
        ...
        const auto seatPositionValue =
            dataPoints.get(Vehicle.Cabin.Copilot.Seat.Row1.DriverSide.Position)->value();
    ...
    publishToTopic(TOPIC_CURRENT_POSITION_COPILOT, jsonResponse.dump());
}

void SeatAdjusterApp::onSetCoPilotPositionRequestReceived(const std::string& data) {
    // Callback is executed whenever a message is received on the subscribed topic
    // The data parameter contains the message payload
    // Payload format: {"requestId": 1, "position": 1}
    ...
    if (!jsonData.contains(JSON_FIELD_POSITION)) {
        ...
        nlohmann::json respData({{JSON_FIELD_REQUEST_ID, jsonData[JSON_FIELD_REQUEST_ID]},
                                 {JSON_FIELD_STATUS, STATUS_FAIL},
                                 {JSON_FIELD_MESSAGE, errorMsg}});
        publishToTopic(TOPIC_RESPONSE_COPILOT, respData.dump());
        return;
    }
    ...
    if (vehicleSpeed == 0) {
        // Move the seat to the desired position
        Vehicle.Cabin.Copilot.Seat.Row1.DriverSide.Position.set(desiredSeatPosition)->await();
